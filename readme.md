# Youssef Ahrour

Software Engineering Student at 1337 (42 network). into backend, APIs, PostgreSQL, auth, and system design. Looking for a software engineering internship or new-grad role.

## Skills & Tools

**Languages:** TypeScript · JavaScript · C/C++ · Java · Bash/Shell

**Frontend:** React · Next.js · TanStack Query · Tailwind CSS · HTML/CSS

**Backend:** Node.js · Express · NestJS · REST APIs · JWT/Auth

**Infrastructure:** PostgreSQL · Docker · AWS S3 · Linux · Nginx

**Tools:** Git/GitHub

## Things I've built

**[ReceiptFlow](https://github.com/yahrour/receiptflow)** AI-powered receipt organizer that uses Google Gemini 2.5 Flash to extract and organize merchant details, amounts, and dates from receipts.

**[Digital Closet](https://github.com/yahrour/digital-closet)** A digital wardrobe manager that helps you organize your clothes, track items, and create outfits from your closet with ease.

## Open Source

Fixed an OAuth bug in a 100+ star project. OAuth users were landing in the wrong password-reset flow. Traced and patched the split without breaking existing flows.
[View the PR](https://github.com/nizzyabi/platform/pull/31)

## Find me here

[Portfolio](https://yahrour.github.io/) · [Email](mailto:youssefahrour.contact@gmail.com)
